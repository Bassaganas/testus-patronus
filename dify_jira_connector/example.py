...
from dify_jira_connector.dify_integration import DifyIntegration
import os
...
def check_env_vars():
    ...
    required_vars = ['JIRA_SERVER_URL', 'JIRA_EMAIL', 'JIRA_API_TOKEN', 'JIRA_PROJECT_NAME_LIST']
    missing_vars = []
    ...
    if missing_vars:
        ...
        if not env_path.exists():
            ...
            logger.error("JIRA_PROJECT_NAME_LIST=project1, project2")
...
def main():
    try:
        ...
        jira_project_name_list = os.getenv('JIRA_PROJECT_NAME_LIST').split(',')

        for jira_project_name in jira_project_name_list:
            jql_query = f"project = {jira_project_name} ORDER BY created DESC"
            issues = jira_client.get_issues(jql_query, max_results=100)
            logger.info(f"Found {len(issues)} issues from {jira_project_name} project")
            if issues:  
